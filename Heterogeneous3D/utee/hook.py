...
def get_operationTemp(area,powerDensity):
    power = int(float(area))*int(float(powerDensity))
    temp = abs(-0.0849*int(float(power))**2+3.0633*int(float(power))+38)+273
    return temp

def hardware_evaluation(model,wl_weight,wl_activation,temp,model_name,mode): 
    global model_n, FP
    model_n = model_name
    ...
    f.write('./NeuroSIM/main ./NeuroSIM/NetWork_'+str(model_name)+'.csv '+str(wl_weight)+' '+str(wl_activation)+' '+str(temp)+' ')
